      ...
      if (this.productCategory.id) {
        this.productCategoryService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('boutiqueApp.productCategory.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.productCategoryService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('boutiqueApp.productCategory.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
