...
describe('ProductOrder e2e test', () => {
  ...
  // const productOrderSample = {"quantity":8671,"totalPrice":11599.27};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"whether joyously incidentally","description":"technician","price":14080.12,"productSize":"XXL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-06-04T12:27:19.841Z","status":"COMPLETED","totalPrice":27785.44,"paymentMethod":"IDEAL","paymentReference":"vainly that picturise"},
    }).then(({ body }) => {
      shoppingCart = body;
    });
  });
  ...
  describe('new ProductOrder page', () => {
    ...
    it.skip('should create an instance of ProductOrder', () => {
      cy.get(`[data-cy="quantity"]`).type('4050');
      cy.get(`[data-cy="quantity"]`).should('have.value', '4050');

      cy.get(`[data-cy="totalPrice"]`).type('15843.69');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '15843.69');
