...
describe('ShoppingCart e2e test', () => {
  ...
  // const shoppingCartSample = {"placedDate":"2024-06-04T00:31:48.309Z","status":"COMPLETED","totalPrice":21937.03,"paymentMethod":"CREDIT_CARD"};
  ...
  describe('new ShoppingCart page', () => {
    ...
    it.skip('should create an instance of ShoppingCart', () => {
      cy.get(`[data-cy="placedDate"]`).type('2024-06-04T02:22');
      cy.get(`[data-cy="placedDate"]`).blur();
      cy.get(`[data-cy="placedDate"]`).should('have.value', '2024-06-04T02:22');

      cy.get(`[data-cy="status"]`).select('PAID');

      cy.get(`[data-cy="totalPrice"]`).type('20026.59');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '20026.59');

      cy.get(`[data-cy="paymentMethod"]`).select('IDEAL');

      cy.get(`[data-cy="paymentReference"]`).type('till upend initiative');
      cy.get(`[data-cy="paymentReference"]`).should('have.value', 'till upend initiative');
